...
const AVAXAxialJLPContract = "0x5305A6c4DA88391F4A9045bF2ED57F4BF0cF4f62";
const AVAXVTXJLPContract = "0x15cbCfafEB3a1B7Ce27A573A522A9956b27fB617";
...
const retrieveAXIALPrice = async () => {
  let provider = new ethers.providers.JsonRpcProvider(Constants.RPCURL);
  const AxialAVAXPool = new ethers.Contract(AVAXAxialJLPContract, JOE_LP_ABI, provider);

  //Primitive way to get AXIAL Price through chain before it get listed in cg or has its own graph
  const AVAXPrice = await retrieveAVAXPrice();
  const reserves = await AxialAVAXPool.getReserves();
  const AXIALQt = reserves._reserve1;
  const AVAXQt = reserves._reserve0;
  return ((AVAXQt / AXIALQt) * AVAXPrice);
}

const retrieveVTXPrice = async () => {
  let provider = new ethers.providers.JsonRpcProvider(Constants.RPCURL);
  const VTXAVAXPool = new ethers.Contract(AVAXVTXJLPContract, JOE_LP_ABI, provider);

  //Primitive way to get VTX Price through chain before it get listed in cg or has its own graph
  const AVAXPrice = await retrieveAVAXPrice();
  const reserves = await VTXAVAXPool.getReserves();
  const VTXQt = reserves._reserve1;
  const AVAXQt = reserves._reserve0;
  return ((AVAXQt / VTXQt) * AVAXPrice);
}
